...
const PERM_ROLES={
  admin:{dashboard:true,pedidos:true,clientes:true,colaboradores:true,produtos:true,financeiro:true,caixa:true,relatorios:true,configuracoes:true},
  comercial:{dashboard:true,pedidos:true,clientes:true,colaboradores:false,produtos:true,financeiro:false,caixa:true,relatorios:false,configuracoes:false},
  financeiro:{dashboard:true,pedidos:true,clientes:true,colaboradores:false,produtos:false,financeiro:true,caixa:true,relatorios:true,configuracoes:false},
  producao:{dashboard:true,pedidos:true,clientes:false,colaboradores:false,produtos:false,financeiro:false,caixa:false,relatorios:false,configuracoes:false},
};
...
// ── CLIENTES ──────────────────────────────────────────────────────────────────
const Clientes=({clientes,setClientes,canEdit,t})=>{
    ...
    <div style={{background:t.surface,border:`1px solid ${t.border}`,borderRadius:12,overflow:"auto"}}>
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
        ...
        <tbody>{filtered.map(c=><tr key={c.id} style={{borderTop:`1px solid ${t.border}`}}>
          ...
          <td style={{padding:"7px 11px"}}>{canEdit&&<div style={{display:"flex",gap:5}}><Btn t={t} variant="ghost" style={{padding:"3px 7px"}} onClick={()=>{setForm(c);setModal("form");}}><Ico n="edit" s={12}/></Btn><Btn t={t} variant="danger" style={{padding:"3px 7px"}} onClick={()=>setClientes(cs=>cs.filter(x=>x.id!==c.id))}><Ico n="trash" s={12}/></Btn></div>}</td>
        </tr>)}</tbody>
      </table>
    </div>
...
// ── COLABORADORES ─────────────────────────────────────────────────────────────
const Colaboradores=({colabs,setColabs,t})=>{
...
// ── PRODUTOS ──────────────────────────────────────────────────────────────────
const Produtos=({produtos,setProdutos,t})=>{
...
// ── PEDIDOS ───────────────────────────────────────────────────────────────────
const Pedidos=({user,pedidos,setPedidos,clientes,colabs,produtos,canEdit,canStatus,t,setFin,setCaixa,addNotif,empresa})=>{
  ...
  const confirmPgto=({valor,forma,destino})=>{const p=selected;const novoVP=(p.valorPago||0)+valor;const novoSt=novoVP>=p.totalPedido?"Pago":novoVP>0?"Parcial":"Pendente";const cl=clientes.find(c=>c.id===p.clienteId);const desc=`Pedido #${p.id} — ${cl?.nome||""} (${forma})`;if(destino==="caixa"){setCaixa(cs=>{const hoje=todayStr();const idx=cs.findIndex(c=>c.data===hoje&&!c.fechado);const m={id:Date.now(),descricao:desc,valor,formaPgto:forma,hora:nowTime(),tipo:"entrada",pedidoId:p.id};if(idx>=0){const up=[...cs];const cx={...up[idx]};cx.movimentos=[...(cx.movimentos||[]),m];cx.totalEntradas=(cx.totalEntradas||0)+valor;cx.totalVendas=(cx.totalVendas||0)+valor;up[idx]=cx;return up;}return[...cs,{id:Date.now()+1,data:hoje,saldoAbertura:0,movimentos:[m],totalEntradas:valor,totalSaidas:0,totalVendas:valor,fechado:false}];});}else{setFin(fs=>{const idx=fs.findIndex(f=>f.pedidoId===p.id&&f.tipo==="receber");if(idx>=0){const up=[...fs];up[idx]={...up[idx],pago:true};return up;}return[...fs,{id:Date.now(),tipo:"receber",descricao:desc,valor,custo:p.totalCusto||0,vencimento:todayStr(),pago:true,pedidoId:p.id}];});}setPedidos(ps=>ps.map(x=>x.id===p.id?{...x,valorPago:novoVP,statusPgto:novoSt}:x));setModal(null);};
  ...
  return <div style={{display:"flex",minHeight:"100vh",background:t.bg,fontFamily:"system-ui,-apple-system,sans-serif",color:t.text}}>
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0}}>
      ...
      <div style={{flex:1,padding:18,overflowY:"auto"}}>
        ...
        {page==="clientes"&&perm.clientes&&<Clientes clientes={clientes} setClientes={setClientesState} canEdit={canEdit} t={t}/>}
        {page==="colaboradores"&&perm.colaboradores&&<Colaboradores colabs={colabs} setColabs={setColabsState} t={t}/>}
        {page==="produtos"&&perm.produtos&&<Produtos produtos={produtos} setProdutos={setProdutosState} t={t}/>}
        {page==="pedidos"&&perm.pedidos&&<Pedidos user={user} pedidos={pedidos} setPedidos={setPedidosState} clientes={clientes} colabs={colabs} produtos={produtos} canEdit={canEdit} canStatus={canStatus} t={t} setFin={setFinState} setCaixa={setCaixaState} addNotif={addNotif} empresa={empresa}/>}
        {page==="financeiro"&&perm.financeiro&&<Financeiro fin={fin} setFin={setFinState} pedidos={pedidos} clientes={clientes} colabs={colabs} t={t}/>}
